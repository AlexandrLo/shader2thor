...
// colour_1, colour_2, contrast and uZoom are supplied per-variation as uniforms
// (see shaders/loops.json: balatro/still). This shader folds the static
// arcana/spectral/felt balatro shaders into one: felt differs only by a geometry
// zoom (uZoom, applied as `uv *= uZoom;` below; 1.0 = no-op for arcana/spectral),
// a lower contrast, and a colour_1 that pre-multiplies the *0.75 factor.
