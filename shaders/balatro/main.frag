...
#define colour_2 vec4(0.0,157./255.,1.,1.0)
...
// so the whole frame repeats exactly every ANIM_PERIOD seconds. main.frag has
// an actual vortex rotation (spin_amount != 0, unlike the arcana/spectral/felt
// balatro shaders), whose natural speed needs a much longer period to fit
// well, hence ANIM_PERIOD=120 here instead of the 26 used elsewhere.
// spin_amount was nudged from the original 0.4 to 0.5 (same magnitude as the
// fish/flint/ox/serpent/wall vortex shaders, same +sign so rotation direction
// is unchanged) so the same harmonic fit applies here too.
#define ANIM_PERIOD 120.0
#define ANIM_OMEGA (6.283185307179586/ANIM_PERIOD)
#define ROT_FREQ (ANIM_OMEGA*1.0)     // k=1 harmonic, closest fit to vortex speed (0.05 rad/s)
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	float shine = (0.3*max(c1p*5. - 4., 0.) + 0.4*max(c2p*5. - 4., 0.));
